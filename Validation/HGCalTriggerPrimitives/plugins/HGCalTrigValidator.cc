...
class HGCalTrigValidator : public DQMEDAnalyzer {

  public:
    explicit HGCalTrigValidator(const edm::ParameterSet&);
    ~HGCalTrigValidator() override;
...
HGCalTrigValidator::HGCalTrigValidator(const edm::ParameterSet& iConfig){
...
HGCalTrigValidator::~HGCalTrigValidator(){
}

void HGCalTrigValidator::bookHistograms(DQMStore::IBooker &iBooker, edm::Run const &, edm::EventSetup const &) {
...
void HGCalTrigValidator::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup){
  using namespace edm;
...
DEFINE_FWK_MODULE(HGCalTrigValidator);
